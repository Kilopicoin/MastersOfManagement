...
contract RoM{
    ...
    struct Realms{
        ...
        uint256 underProduction; // 0 for none, 1 for underProduction
        uint256 underProductionName; // 1 WoodenClub 2 StoneAxe 3 StoneSpear 4 StoneArrowBow
        uint256 underProductionQuantity;
        uint256 underTraining; // 0 for none, 1 for underProduction
        uint256 underTrainingName;
        uint256 underTrainingQuantity;
        uint256 underAttack; // 0 for none, 1 for underProduction
...
struct RealmsWarRecords{
        ...
        address Defender;
        uint256 Date;
        uint256 Result; // 1 for win, 2 for lose
    }
...
//     finishWorld = block.timestamp + ( lifeWorld * 600 );
    ...
    struct useTurnG{
        ...
        uint256 product; // 1 WoodenClub 2 StoneAxe 3 StoneSpear 4 StoneArrowBow
        uint256 productQuantity;
        uint256 productManPower;
        uint256 warrior; // 1 WoodenClubFighter 2 StoneAxeFighter 3 StoneSpearFighter 4 StoneArrowBowFighter
	...
	function addRealm(string memory _name) public {
		...
		RealmNo[RealmCount] = Realms(300,block.timestamp,10,_name,msg.sender,2000,0,1000000,0,0,10000000,0,0,10000000,0,0,10000000,0,0,0,0,0,0,0,0,msg.sender,0);
//      RealmNo[RealmCount] = Realms(5005,block.timestamp,10,_name,msg.sender,2000,0,1000000,0,0,10000000,0,0,10000000,0,0,10000000,0,0,0,0,0,0,0,0,msg.sender,0);
...
RealmNo[realmnum].popRate = useTurnGx.popRatex;
        ...
        if ( RealmNo[realmnum].food > popH * useTurnGx.turnUse / 5) {

            if ( RealmNo[realmnum].food > 1000000) {

            RealmNo[realmnum].food = RealmNo[realmnum].food - ( popH * useTurnGx.turnUse  / 5 );
        ...
        RealmNo[realmnum].food = RealmNo[realmnum].food + (( RealmNo[realmnum].foodRate * useTurnGx.turnUse * popH) / 1000 );
...
RealmNoWarRecords[realmnumD][RealmNoWarCount[realmnumD]].Result = 1;
    ...
    if ( RealmNoWarriors[realmnumD].WoodenClubFighter > 10) {
        ...
        RealmNoWarriors[realmnumD].WoodenClubFighter = RealmNoWarriors[realmnumD].WoodenClubFighter - (( RealmNoWarriors[realmnumD].WoodenClubFighter * 10 ) / 100 );
        
    } else if ( RealmNoWarriors[realmnumD].WoodenClubFighter > 1) {
    ...
    if ( RealmNoWarriors[realmnumD].StoneAxeFighter > 10) {
        ...
        RealmNoWarriors[realmnumD].StoneAxeFighter = RealmNoWarriors[realmnumD].StoneAxeFighter - (( RealmNoWarriors[realmnumD].StoneAxeFighter * 10 ) / 100 );
        
    } else if ( RealmNoWarriors[realmnumD].StoneAxeFighter > 1) {
    ...
    if ( RealmNoWarriors[realmnumD].StoneSpearFighter > 10) {
        ...
        RealmNoWarriors[realmnumD].StoneSpearFighter = RealmNoWarriors[realmnumD].StoneSpearFighter - (( RealmNoWarriors[realmnumD].StoneSpearFighter * 10 ) / 100 );
        
    } else if ( RealmNoWarriors[realmnumD].StoneSpearFighter > 1) {
    ...
    if ( RealmNoWarriors[realmnumD].StoneArrowBowFighter > 10) {
        ...
        RealmNoWarriors[realmnumD].StoneArrowBowFighter = RealmNoWarriors[realmnumD].StoneArrowBowFighter - (( RealmNoWarriors[realmnumD].StoneArrowBowFighter * 10 ) / 100 );
        
    } else if ( RealmNoWarriors[realmnumD].StoneArrowBowFighter > 1) {
...
if ( RealmNoAttackers[realmnum].WoodenClubFighter > 10) {
    ...
    } else if ( RealmNoAttackers[realmnum].WoodenClubFighter > 1) {
    ...
    } else if ( RealmNoAttackers[realmnum].StoneAxeFighter > 1) {
    ...
    } else if ( RealmNoAttackers[realmnum].StoneSpearFighter > 1) {
    ...
    } else if ( RealmNoAttackers[realmnum].StoneArrowBowFighter > 1) {
...
if ( RealmNoAttackers[realmnum].WoodenClubFighter > 10) {
        ...
        RealmNoAttackers[realmnum].WoodenClubFighter = RealmNoAttackers[realmnum].WoodenClubFighter - (( RealmNoAttackers[realmnum].WoodenClubFighter * 10 ) / 100 );
    ...
    if ( RealmNoAttackers[realmnum].StoneAxeFighter > 10) {
        ...
        RealmNoAttackers[realmnum].StoneAxeFighter = RealmNoAttackers[realmnum].StoneAxeFighter - (( RealmNoAttackers[realmnum].StoneAxeFighter * 10 ) / 100 );
    ...
    if ( RealmNoAttackers[realmnum].StoneSpearFighter > 10) {
        ...
        RealmNoAttackers[realmnum].StoneSpearFighter = RealmNoAttackers[realmnum].StoneSpearFighter - (( RealmNoAttackers[realmnum].StoneSpearFighter * 10 ) / 100 );
    ...
    if ( RealmNoAttackers[realmnum].StoneArrowBowFighter > 10) {
        ...
        RealmNoAttackers[realmnum].StoneArrowBowFighter = RealmNoAttackers[realmnum].StoneArrowBowFighter - (( RealmNoAttackers[realmnum].StoneArrowBowFighter * 10 ) / 100 );
...
if ( RealmNo[realmnumD].food > 100000) {
        ...
        uint256 yemekKazanc = ( RealmNo[realmnumD].food * 30 ) / 100;
        RealmNo[realmnumD].food = RealmNo[realmnumD].food - yemekKazanc;
        RealmNo[realmnum].food = RealmNo[realmnum].food + yemekKazanc;
...
if ( RealmNo[realmnumD].wood > 100000) {
        ...
        uint256 yemekKazanc = ( RealmNo[realmnumD].wood * 30 ) / 100;
        RealmNo[realmnumD].wood = RealmNo[realmnumD].wood - yemekKazanc;
        RealmNo[realmnum].wood = RealmNo[realmnum].wood + yemekKazanc;
...
if ( RealmNo[realmnumD].stone > 100000) {
        ...
        uint256 yemekKazanc = ( RealmNo[realmnumD].stone * 30 ) / 100;
        RealmNo[realmnumD].stone = RealmNo[realmnumD].stone - yemekKazanc;
        RealmNo[realmnum].stone = RealmNo[realmnum].stone + yemekKazanc;
        ...
        RealmNo[realmnum].stone = RealmNo[realmnum].stone + yemekKazanc;
            ...
            if (RealmNo[realmnum].underProduction == 1 && useTurnGx.cancelproduction == 1) {
                ...
                if ( useTurnGx.productQuantity != 0) {
                    ...
                    if ( (RealmNo[realmnum].popT * RealmNo[realmnum].productionRate) / 100000 >= useTurnGx.productManPower ) {
                        ...
                        RealmNoProducing[realmnum] = ( useTurnGx.productQuantity * 30 ) / useTurnGx.productManPower;
                        ...
                        RealmNoProducing[realmnum] = ( useTurnGx.productQuantity * 50 ) / useTurnGx.productManPower;
                        RealmNo[realmnum].wood = RealmNo[realmnum].wood - ( useTurnGx.productQuantity * 100000 );
                        ...
                        RealmNoProducing[realmnum] = ( useTurnGx.productQuantity * 60 ) / useTurnGx.productManPower;
                        RealmNo[realmnum].wood = RealmNo[realmnum].wood - ( useTurnGx.productQuantity * 100000 );
...
if ( RealmNo[realmnum].underTraining == 1) {
        ...
        RealmNoResearchRate[realmnum][useTurnGx.res1] = useTurnGx.res1am;
        ...
        RealmNoResearchRate[realmnum][useTurnGx.res2] = useTurnGx.res2am;
        ...
        RealmNoResearchRate[realmnum][useTurnGx.res3] = useTurnGx.res3am;
...
// nufusyemekoduntaskontrol
        ...
        RealmNo[realmnum].points = RealmNo[realmnum].points + ( RealmNoProducts[realmnum].WoodenClub * 250);
        RealmNo[realmnum].points = RealmNo[realmnum].points + ( RealmNoProducts[realmnum].StoneAxe * 400);
        RealmNo[realmnum].points = RealmNo[realmnum].points + ( RealmNoProducts[realmnum].StoneSpear * 380);
        RealmNo[realmnum].points = RealmNo[realmnum].points + ( RealmNoProducts[realmnum].StoneArrowBow * 300);
        ...
        RealmNo[realmnum].points = RealmNo[realmnum].points + ( RealmNoWarriors[realmnum].StoneSpearFighter * 720);
